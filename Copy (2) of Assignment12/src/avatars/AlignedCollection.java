...
AvatarCollectable {
	...
	public void moveAnimatedX(int newVal) {
		moveX = moveX + newVal;
		setX(moveX);
		for (int i=0; i<size; i++){
			((Talkable)objectArray[i]).moveAnimatedX(newVal);

		}
	}

	public void moveAnimatedY(int newVal) {
		moveY = moveY + newVal;
		setY(moveY);
		for (int i=0; i<size; i++){
			((Talkable)objectArray[i]).moveAnimatedY(newVal);
		}
	}

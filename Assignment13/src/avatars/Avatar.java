...
public abstract class Avatar implements Avatarized{
	...
	public void setAnimatedX(int x) {
		xValue = x;
		//buildAvatar();
		body.setAnimatedX(xValue);
		head.setAnimatedX(xValue);
		leftArm.setAnimatedX(xValue);
		rightArm.setAnimatedX(xValue + bodyWidth);
		leftLeg.setAnimatedX(xValue + headWidth / 3);
		rightLeg.setAnimatedX(xValue + (2 * headWidth / 3));
		name.setAnimatedX(xValue);
	
	}

	
	public void setAnimatedY(int y) {
		yValue = y;
		body.setAnimatedY(yValue + headHeight);
		head.setAnimatedY(yValue);
		leftArm.setAnimatedY(yValue + headHeight * 2);
		rightArm.setAnimatedY(yValue + headHeight * 2);
		leftLeg.setAnimatedY(yValue + bodyHeight);
		rightLeg.setAnimatedY(yValue + bodyHeight);
		name.setAnimatedY(yValue-20);
	}

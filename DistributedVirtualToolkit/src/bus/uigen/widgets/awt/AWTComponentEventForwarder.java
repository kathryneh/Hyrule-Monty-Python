...
public class AWTComponentEventForwarder implements FocusListener,MouseListener, MouseMotionListener {
	...
	public void mouseClicked(MouseEvent event) {
		...
		if(currTime-timeLastClick < timeForDoubleClick){
			virtualEvent = AWTEventPackager.convert(event,VirtualMouseEvent.MouseDoubleClick);
			for(int i=0; i<listeners.size(); i++){
	...
	public void mouseDragged(MouseEvent event) {
		VirtualMouseEvent virtualEvent = AWTEventPackager.convert(event, VirtualMouseEvent.Mouse_Drag);
		Vector<VirtualMouseMoveListener> listeners = component.getVirtualMouseMoveListener();
		for(int i=0; i<listeners.size(); i++){
	...
	public void mouseMoved(MouseEvent event) {
		VirtualMouseEvent virtualEvent = AWTEventPackager.convert(event, VirtualMouseEvent.Mouse_Move);
		Vector<VirtualMouseMoveListener> listeners = component.getVirtualMouseMoveListener();
		for(int i=0; i<listeners.size(); i++){

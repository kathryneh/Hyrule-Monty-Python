...
public class AWTFrame extends AWTContainer implements VirtualFrame {
	...
	*/
    ...
    public void  setContentPane(VirtualContainer container) {
    	//getFrame().setContentPane((Container) container.getPhysicalComponent());
    }
    public void pack() {
    	getFrame().pack();
    }
	...
	public AnAWTFrame (Frame theFrame) {
		super (theFrame);
		frame = theFrame;
    ...
    public void setMenuBar(Object newVal) {
    	frame.setMenuBar ((MenuBar) newVal);
    }

	public void setContentPane(VirtualContainer container) {
		frame.add((Container) container.getPhysicalComponent());
		System.out.println("setContentPane: cannot change container, adding to existing content pane");
	}
	
    public void addWindowListener(Object newVal) {
    	frame.addWindowListener((WindowListener) newVal );
    }
    public void addComponentListener (ComponentListener cl) {
	...
	//static transient Hashtable<Container, VirtualContainer> containersToVirtualContainers = new Hashtable();	
	public static VirtualFrame virtualFrame (Frame f) {	
		return (VirtualFrame) AnAWTComponent.virtualComponent(f);

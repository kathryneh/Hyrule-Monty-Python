...
public class AUniversalWidget  {
	...
	public static UniversalWidget universalWidget (Object c) {
		...
		if (vc == null) {
			try {
			//Class virtualClass = componentClassToUniversalWidgetClass.get(c.getClass());
			Class virtualClass = getVirtualClass(c.getClass());
			if (virtualClass == null) vc = null;
			else {
			...
			/*else if (c instanceof Container)
				vc = new AnAWTContainer((Container) c);
			else if (c instanceof Frame)
				vc = new AnAWTFrame ((Frame) c);
			else if (c instanceof JTextField)
				vc = new ASwingTextField ((JTextField) c);
			else if (c instanceof JButton)
				vc = new ASwingButton ((JButton) c);
			else if (c instanceof JSlider)
				vc = new ASwingSlider ((JSlider) c);
			else  if (c instanceof JTree)
				vc = new ASwingTree ((JTree) c);
			else

...
//import bus.uigen.widgets.swt.SWTMenuBar.MenuData;
...
public class SWTMenu extends SWTMenuComponent implements VirtualMenu {
	//Menu menu;
	//MenuData md = new MenuData(); //should we not create a new MenuData() here?
	MenuData md;
	class MenuData { //could this be static?
		...
		String text;
		Menu menuToSet; //to store the menu reference if the physical component doesn't exist yet
		...
		String getText() { //TODO is this appropriate? Menus don't have text fields in SWT
			return text;
		}
		...
		/*void insertItem(MenuItem mi, int pos) { //is this ever used? probably not: deprecated
			SWTMenuItem m = new SWTMenuItem(mi);
			items.insertElementAt(m, pos);
		}*/
		...
		/*void removeItem(MenuItem mi) { //is this used? probably not: deprecated
			for (int i=0;i<items.size();i++) {
		...
		void removeItem(SWTMenuItem swtMI) {
			for (int i=0;i<items.size();i++) {
				if (items.get(i).equals(swtMI)==true) { //will this work to compare two SWTMenuItems?
					items.remove(i);
					break;
				}
			}
		...
		void setMenuToSet(Menu theMenu) {
			menuToSet = theMenu; //can't create new Menu here without referencing the parent			
		}
		Menu getMenuToSet() {
			return menuToSet;
		}
	...
	public SWTMenu (Menu theMenu) {
		//super (theMenu);
		md = new MenuData();
		/*for (int i=0; i<theMenu.getItemCount(); i++) { //is this valid if theMenu component doesn't exist yet? i think not..
			md.addItem(theMenu.getItem(i));
		}*/
		md.setMenuToSet(theMenu);
		//we can't add the items yet because the menu component probably doesn't exist.
	...
	public Menu create(Menu parent) {
		...
		if (getMenuToSet()!=null) { //if there is a menu to set
			menu = getMenuToSet(); //is this valid? it overrides all other data stored in md
	...
	*/
	...
	public void add (VirtualMenuItem c) {
		...
		//String text = new String(c.getText()); //error at TestSWT,74: was ((SWTMenuItem) c)
		if (c instanceof SWTMenuItem) {
			md.addItem((SWTMenuItem) c); //will need to be able to add SWTMenuItem c and SWTMenu c here
		}
	...
	public void insert(VirtualMenuItem c, int pos) {
		...
		//md.insertItem((MenuItem)c.getPhysicalComponent(), pos); //is the position usage the same? (0-based)
		//md.insertItem(((SWTMenuItem) c).getItem(), pos);
		//will this try to reference the physical component before it exists also?
		
		if (c instanceof SWTMenuItem) {
			md.insertItem((SWTMenuItem) c, pos); //will need to be able to add SWTMenuItem c and SWTMenu c here
		}
		else if (c instanceof SWTMenu) {
	...
	public void remove (VirtualMenuItem c) {
		...
		//md.removeItem((MenuItem)c.getPhysicalComponent()); //see usage as above in insert
			//does this reference the physical component before it exists?
		
		if (c instanceof SWTMenuItem) {
			md.removeItem((SWTMenuItem) c); //will need to be able to add SWTMenuItem c and SWTMenu c here
		}
		else if (c instanceof SWTMenu) {
	...
	public void setMenuToSet(Menu m) {
		/*if (md.menuToSet!=null) md.setMenuToSet(m); //a menu reference has been saved to be set later because it doesn't exist yet
			//when menu is created from menuToSet, menuToSet must be returned to null
		else {
	...
	public void setEnabled (boolean newVal) {
		getMenu().setEnabled(newVal);
	}
	public void enable () {
		setEnabled(true);
	}
	public void postEvent(Event event) {
		getMenu().postEvent(event);
	}
	public void setLabel (String newVal) {
		getMenu().setLabel(newVal);
	}
	...
	*/
	...
	public static SWTMenu virtualMenu (Menu theMenu) {
		return (SWTMenu) AUniversalWidget.universalWidget(theMenu);
	}
	public void enable() {

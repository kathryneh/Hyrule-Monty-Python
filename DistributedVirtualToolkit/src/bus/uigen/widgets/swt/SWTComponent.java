...
public abstract class SWTComponent extends AUniversalWidget implements VirtualComponent {
	...
	int width=-1, height=-1; //conceivably someone might want to set width or height to 0
	boolean toPack = false; //if object is set to be packed when it is created
	
	Vector<VirtualGraphic> graphics = new Vector<VirtualGraphic>();
	...
	public void init(){
		SWTComponentEventForwarder forwarder = new SWTComponentEventForwarder(this);
	...
	*/
	...
	public void setLocation(int newX, int newY) {
		//getComponent().setLocation(newX, newY); //component does not yet exist
		xLocation = newX;
		yLocation = newY;
	}
	...
	public void setSize (int newWidth, int newHeight) {
		...
		if(VirtualToolkit.isDistributed()){
			...
			VirtualToolkit.sendCommand(COMMAND_LABEL + widgetID + SET_SIZE_COMMAND + uniqueID + "," + newWidth + "," + newHeight + ")");
		}
	}
	...
	public void execSetSize (int newWidth, int newHeight){
		//getComponent().setSize(newWidth, newHeight); //impossible for Button, TextField because the component does not exist yet
	...
	*/
	 ...
	 * 
	 * This indicates a component has been initialized to have component added to it
	 */

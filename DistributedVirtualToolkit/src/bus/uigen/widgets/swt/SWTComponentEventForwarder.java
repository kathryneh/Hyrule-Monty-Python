...
public class SWTComponentEventForwarder implements FocusListener,MouseTrackListener, MouseListener, MouseMoveListener {
	...
	public void focusGained(FocusEvent event) {
		...
		Vector<VirtualFocusListener> listeners = component.getVirtualFocusListeners();
	...
	public void mouseEnter(MouseEvent event) {
		inside=true;
		VirtualMouseEvent virtualEvent = SWTEventPackager.convert(event, VirtualMouseEvent.MouseEnter);
		Vector<VirtualMouseListener> listeners = component.getVirtualMouseListeners();
		for(int i=0; i<listeners.size(); i++){
	...
	public void mouseExit(MouseEvent event) {
		inside=false;
		VirtualMouseEvent virtualEvent = SWTEventPackager.convert(event, VirtualMouseEvent.MouseExit);
		Vector<VirtualMouseListener> listeners = component.getVirtualMouseListeners();
		for(int i=0; i<listeners.size(); i++){
	...
	public void mouseHover(MouseEvent event) {
		VirtualMouseEvent virtualEvent = SWTEventPackager.convert(event, VirtualMouseEvent.MouseHover);
		Vector<VirtualMouseListener> listeners = component.getVirtualMouseListeners();
		for(int i=0; i<listeners.size(); i++){
	...
	public void mouseDoubleClick(MouseEvent event) {
		VirtualMouseEvent virtualEvent = SWTEventPackager.convert(event, VirtualMouseEvent.MouseDoubleClick);
		Vector<VirtualMouseListener> listeners = component.getVirtualMouseListeners();
		for(int i=0; i<listeners.size(); i++){
	...
	public void mouseDown(MouseEvent event) {
		down = true;
		VirtualMouseEvent virtualEvent = SWTEventPackager.convert(event, VirtualMouseEvent.MouseDown);
		Vector<VirtualMouseListener> listeners = component.getVirtualMouseListeners();
		for(int i=0; i<listeners.size(); i++){
	...
	public void mouseUp(MouseEvent event) {
		...
		down = false;
		VirtualMouseEvent virtualEvent = SWTEventPackager.convert(event, VirtualMouseEvent.MouseUp);
		Vector<VirtualMouseListener> listeners = component.getVirtualMouseListeners();
		for(int i=0; i<listeners.size(); i++){
		...
		if(click){
			virtualEvent = SWTEventPackager.convert(event, VirtualMouseEvent.MouseClick);
			for(int i=0; i<listeners.size(); i++){
	...
	public void mouseMove(MouseEvent event) {
		if(!down){
			VirtualMouseEvent virtualEvent = SWTEventPackager.convert(event, VirtualMouseEvent.Mouse_Move);
			Vector<VirtualMouseMoveListener> listeners = component.getVirtualMouseMoveListeners();
			for(int i=0; i<listeners.size(); i++){
				listeners.elementAt(i).mouseMoved(virtualEvent);
			}
		}else{
			VirtualMouseEvent virtualEvent = SWTEventPackager.convert(event, VirtualMouseEvent.Mouse_Drag);
			Vector<VirtualMouseMoveListener> listeners = component.getVirtualMouseMoveListeners();
			for(int i=0; i<listeners.size(); i++){

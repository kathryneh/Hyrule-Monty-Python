...
public class SWTTable extends SWTContainer implements VirtualTable{
	...
	public void addToParent (VirtualContainer theParent) {		
		...
		component = new Table ((Composite)theParent.getPhysicalComponent(), SWT.BORDER);
	...
	private void buildComponent(){
		if(getComponent() != null){
			...
			if(model!=null){
				...
				TableItem currRow = new TableItem((Table) component, SWT.NONE);
					...
					for(int j=0; j<model.getColumnCount(); j++){
						if(model.useTextForColumn(j)){
							if(model.getValueAt(i, j)==null){
								...
								rowTextData[j] = model.getValueAt(i, j).toString();
							}
							rowImageData[j] = null;
						}else{
							rowTextData[j] = "";
	...
	public void updateCellChanged(int row, int column){
		if(getComponent() != null ){
			if(model.useTextForColumn(column)){
				getTable().getItem(row).setText(column, model.getValueAt(row, column).toString());
			}else{
				getTable().getItem(row).setImage(column, (Image) model.getValueAt(row, column));
			}
		}
	...
	public void setColumnWidth(int colNum, int width) {
		if(component != null){
			...
			if(deltaWidth>0 || (deltaWidth<0 && getTable().getBounds().width>deltaWidth)){
				getTable().setSize(getTable().getBounds().width+deltaWidth, getTable().getBounds().height);
			}*/

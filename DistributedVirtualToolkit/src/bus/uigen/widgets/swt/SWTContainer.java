...
public abstract class SWTContainer extends SWTComponent implements VirtualContainer {
	...
	public void execAdd(VirtualComponent c){
		...
		if (component == null){
			...
			this.addChild(children.size(), c);
			//c.addToParent(this);
			this.getComponent().getDisplay().asyncExec(new AddsToParent(this, c));
	...
	public void add (VirtualComponent c, Object constraint) {
		/*
		//getContainer().add(((AnSWTComponent) c).getSWTComponent(), constraint);
	...
	public void add (VirtualComponent c, Object constraint, int pos) {
		/*
		//getContainer().add(((AnSWTComponent) c).getSWTComponent(), constraint, pos);
		getContainer().add((Component) (c.getPhysicalComponent()), constraint, pos);
		c.setParent(this);
		*/
	...
	public void remove (VirtualComponent c) {
		...
		/*children.remove(c); //is this valid in all cases? see add() method above
		c.setParent(null); //what am i doing here?
		c.invalidate(); //what does this do?
*/	}
	...
	public VirtualComponent[] getComponents() {
		...
		for (int i = 0; i < components.length; i++)
			retVal[i] = SWTComponent.virtualComponent(components[i]);
		return retVal;
		*/
	}
	public VirtualComponent getComponent(int pos) {
		return null;
		/*
		return SWTComponent.virtualComponent(getContainer().getComponents()[pos]);
		*/
	}
	...
	public static VirtualContainer virtualContainer (Composite c) {	
		...
		VirtualComponent vcomp = AnSWTComponent.existingVirtualComponent(c);
		if (c != null)
			return (VirtualContainer) vcomp;
		VirtualContainer vgetContainer() = new AnSWTContainer(c);
		AnSWTComponent.register (c, vgetContainer());
		*/
		/*
			getContainer()sToVirtualContainers.get(c);
		if (vc == null) {
			vc = new AnSWTContainer(c);

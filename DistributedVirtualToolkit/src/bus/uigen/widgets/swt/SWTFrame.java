...
public class SWTFrame extends SWTContainer implements VirtualFrame, VirtualContainer { //this should really be SWTShell
	//JFrame getFrame();
	...
	public SWTFrame (Display theFrame, VirtualContainer theContents) {
		...
		resetContents();
		//contents = AnAWTContainer.virtualContainer(getJFrame().getContentPane());
    ...
    public void  setContentPane(VirtualContainer container) {
    	...
    	//getJFrame().setContentPane((Container) container.getPhysicalComponent());
    	//resetContents();
    	
    }
    void resetContents() {
    	//contents = AWTContainer.virtualContainer(getDisplay().getContentPane());
    }
    public void setMenuBar (VirtualMenuBar theMenuBar) {
		...
		((JFrame) getFrame()).setJMenuBar ((JMenuBar) menuBar.getPhysicalComponent());
    	else super.setMenuBar(menuBar);
    	*/
    	
    	//where does the new Menu() command go here?
    	//perhaps menuBar.create() can return a new Menu, which is then set to menuBar's physical component somehow
    	...
    	//contents.getPhysicalComponent().setMenuBar ((MenuBar) menuBar.getPhysicalComponent());
    	//getShell().setMenuBar((MenuBar) menuBar.getPhysicalComponent()); //has to take Menu
    	//getShell().setMenuBar((Menu) menuBar.getPhysicalComponent()); //is this ok? i probably first have to create the component
    	getShell().setMenuBar(menu);
    	//System.out.println("Menu has been set to Shell");
    	//((SWTMenuBar) menuBar).create(getShell()); //physically define menus and submenus here
	}
    ...
    public void setResizable(boolean newVal) {
    	getJFrame().setResizable(newVal);
    }
    public void setTitle (String label) {
    	getJFrame().setTitle(label);
    }
    ...
    public void setCursor (int newVal) {
		 getJFrame().setCursor(newVal);
	}
    public void dispose() {
		 getJFrame().dispose();
	}
    public void setMenuBar(Object newVal) {
    	getJFrame().setMenuBar ((MenuBar) newVal);
    }
	
    public void addWindowListener(Object newVal) {
    	getJFrame().addWindowListener((WindowListener) newVal );
    }
    public void addComponentListener (ComponentListener cl) {
    	getJFrame().addComponentListener(cl);
    }
	*/
	//static transient Hashtable<Container, VirtualContainer> containersToVirtualContainers = new Hashtable();	
	...
	public static VirtualFrame virtualFrame (Display f) {	
		return (VirtualFrame) AUniversalWidget.universalWidget(f);
		...
		VirtualContainer vcontainer = new AnAWTContainer(c);
		AnAWTComponent.register (c, vcontainer);
		*/
		/*
			containersToVirtualContainers.get(c);
		if (vc == null) {
			vc = new AnAWTContainer(c);
			containersToVirtualContainers.put(c, vc);			
		}
		*/
		//return vcontainer;
	...
	public int getWidth() {
		return ((Dimension) getContentPane().getSize()).width;
	...
	*/
	...
	/*
	public void setLocation(int newX, int newY) {
		 getWidget().setLocation(newX, newY);
	}
	public void setLocation(Point newVal) {
	...
	*/
	...
	private class SetVisibleThread extends Thread{
		boolean val;
		public SetVisibleThread(boolean val){this.val = val;}
		public void run(){
	...
	public void execSetLayout(VirtualLayout l){
		if(!(l==null || l instanceof SWTLayout )){ throw new IllegalArgumentException("Argument must be of SWT type");} 
		layout = (SWTLayout) l;

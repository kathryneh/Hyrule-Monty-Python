...
public class Widget implements VirtualComponent {
	...
	public void addToParent (VirtualContainer theParent) {
		//Element can only be added to a parent of the component has been initialized
		if(component==null){System.out.println("Widget has not been created properly");}
	...
	public void setCursor(Object newVal) {
		component.setCursor((Cursor) newVal);
	}
	 ...
	 * componentsToVirtualComponents.get(c);
	 *  } public static void register (Component c, VirtualComponent vc) {
	 * componentsToVirtualComponents.put(c, vc); } public static
	 * VirtualComponent virtualComponent (Component c) { if (c == null) return
	 * null; VirtualComponent vc = componentsToVirtualComponents.get(c); if (vc ==
	 * null) { if (c instanceof JLabel) vc = new ASwingLabel ((JLabel) c); else
	 * if (c instanceof Container) vc = new AnAWTContainer((Container) c); else
	 * if (c instanceof Frame) vc = new AnAWTFrame ((Frame) c); else if (c
	 * instanceof JTree) vc = new ASwingTree ((JTree) c); else vc = new
	 * AnAWTComponent(c); componentsToVirtualComponents.put(c, vc); } return vc;
	 *  }
	 */

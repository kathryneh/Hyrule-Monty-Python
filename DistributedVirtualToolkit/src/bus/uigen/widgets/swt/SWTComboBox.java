...
public class SWTComboBox extends SWTComponent implements VirtualComboBox {
	...
	public void init (Object[] choices) {
		...
		//super.init(c); //no component is initialized here, only in addToParent		
	}
	...
	public void addToParent(VirtualContainer theParent) {
		...
		if (readOnly) component = new Combo ((Composite)theParent.getPhysicalComponent(), SWT.DROP_DOWN | SWT.READ_ONLY);
		else component = new Combo ((Composite)theParent.getPhysicalComponent(), SWT.DROP_DOWN);
		for (int i=0;i<comboChoices.size();i++) {

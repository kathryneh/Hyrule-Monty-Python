...
public class VirtualMouseEvent{
	...
	public static final int MouseDoubleClick = 8;//SWT.MouseDoubleClick;  //Value is 8
	...
	//private Widget swtWidget = new org.eclipse.swt.widgets.Button(new Shell(), SWT.NONE); // just a filler object
	private int type;
	private long when = -1;
	...
	/*public VirtualMouseEvent(java.awt.event.MouseEvent awtEvent, int type){
		...
		this.type = type;
		this.when = awtEvent.getWhen();
		this.modifiers= awtEvent.getModifiers();
		this.x = awtEvent.getX();
		this.y = awtEvent.getY();
		if(this.type == VirtualMouseAdapter.MouseClick) clickCount = 1;
		if(this.type == VirtualMouseAdapter.MouseDoubleClick) clickCount = 2;
		this.button = awtEvent.getButton();
		this.popUpTrigger = awtEvent.isPopupTrigger();
	}
	
	public VirtualMouseEvent(org.eclipse.swt.events.MouseEvent swtEvent, int type){
		this.swtEvent = swtEvent;
		this.type = type;
		this.when = swtEvent.time;
		this.modifiers = swtEvent.stateMask;
		this.x = swtEvent.x;
		this.y = swtEvent.y;
		if(this.type == VirtualMouseAdapter.MouseClick) clickCount = 1;
		if(this.type == VirtualMouseAdapter.MouseDoubleClick) clickCount = 2;
		this.button = swtEvent.button;
	}*/
	...
	/*public Point getLocationOnScreen() throws PackageMismatchException{
		...
		return awtEvent.getLocationOnScreen();
	}*/
	...
	/*public java.awt.event.MouseEvent getAWTMouseEvent(){
		if(awtEvent!=null){
			...
			/*Component source = new Button(); //just a filler object
			int id = type;  //Need to check for real way to set id in AWT
			long when = (long) swtEvent.time;
			int modifiers = swtEvent.stateMask;
			int x = swtEvent.x;
			int y = swtEvent.y;
			int clickCount = 0;
			if(type == VirtualMouseAdapter.MouseClick) clickCount = 1;
			if(type == VirtualMouseAdapter.MouseDoubleClick) clickCount = 2;
			boolean popUpTrigger = false; //check on popUpTrigger possibility in swt MouseEvent
			int button = swtEvent.button;
			return new java.awt.event.MouseEvent(awtSource, type, when, modifiers, x, y, clickCount, popUpTrigger, button);
		}
	...
	public org.eclipse.swt.events.MouseEvent getSWTMouseEvent(){
		if(swtEvent != null){
			...
			org.eclipse.swt.events.MouseEvent toReturn = new org.eclipse.swt.events.MouseEvent(event);
			
			return toReturn;
		}
	}*/
	
	/*public int button() throws PackageMismatchException{
		if(swtEvent == null) 
			throw new PackageMismatchException("Cannot call button() from the chosen toolkit");
		return swtEvent.button;
	}*/
	
	/*public int x() throws PackageMismatchException{
	if(swtEvent == null) 
		throw new PackageMismatchException("Cannot call x() from the chosen toolkit");
	return swtEvent.x;
	}*/

	/*public int y() throws PackageMismatchException{
		if(swtEvent == null) 
			throw new PackageMismatchException("Cannot call y() from the chosen toolkit");
		return swtEvent.y;
	}*/

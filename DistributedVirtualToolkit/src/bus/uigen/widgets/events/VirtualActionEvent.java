...
public class VirtualActionEvent{
	...
	public static final String FIRE_COMMAND = "fireEvent(";
	/*protected ActionEvent actionComponent;
	protected SelectionEvent selectionComponent;
	protected ModifyEvent modifyComponent;*/
	...
	//org.eclipse.swt.widgets.Button swtSource = new org.eclipse.swt.widgets.Button(new Shell(), SWT.NONE); //just a filler
	
	public VirtualActionEvent(){
		
	}
	
	/*public VirtualActionEvent(ActionEvent event){
		actionComponent = event;
	...
	public VirtualActionEvent(SelectionEvent event, String command){
		selectionComponent = event;
		cmd = command;
		when = event.time;
		modifiers = event.stateMask;
		type = VirtualActionEvent.SELECTED;
	}
	
	public VirtualActionEvent(ModifyEvent event, String command){
		modifyComponent = event;
		cmd = command;
		when = event.time;
		modifiers = -1;
		type = VirtualActionEvent.MODIFIED;
	}*/
	...
	/*public ActionEvent getActionEvent(){
		if(actionComponent != null){
			...
			awtSource.setLabel(cmd);
			return new ActionEvent(awtSource,type,cmd, (int)when, modifiers);
		}
	}*/
	...
	public static void runCommand(String command){
		...
		if(command.startsWith(FIRE_COMMAND)){
			String argsStr = command.substring(FIRE_COMMAND.length(), command.lastIndexOf(')'));
			String[] args = argsStr.split(",");
			if(args[0].equals(VirtualToolkit.getUniqueID())){
				return;
			}
			
			VirtualComponent component = (VirtualComponent) VirtualToolkit.getObjectByID(args[1]);
			...
			if(component instanceof VirtualButton){
				((VirtualButton) component).fireVirtualActionEvent(event);
			}else if(component instanceof VirtualTextField){
				((VirtualTextField) component).fireVirtualActionEvent(event);
			}
		}

...
public class DistributedVirtualToolkit implements EntryPoint {
	...
	static class EvalListener implements VirtualActionListener{
		...
		public void actionPerformed(VirtualActionEvent e) {
			String textToSolve = ((VirtualTextField) VirtualToolkit.getObjectByID(fieldID)).getText();
			String solution = eval(textToSolve);
			((VirtualTextField) VirtualToolkit.getObjectByID(fieldID)).setText(solution);
		}
	...
	static VirtualTextField buildCalcField(){
		...
		field.blockActionEventForwarding(true);
		field.addActionListener(new EvalListener(field));
		return field;
	}
	
	static VirtualContainer buildButtonPanel(String b1, String b2, String b3, String b4){
	...
	static VirtualButton buildCalcButton(String text){
		...
		if(text.equals("=")) button.addActionListener(new EvalListener(field));
		else button.addActionListener(new CalcButtonListener(text));
		return button;
	}
	...
	static void buildGUI(){
		...
		button1.add(buildCalcButton("2"));
		button1.add(buildCalcButton("3"));
		button1.add(buildCalcButton("+"));
		
		button2.add(buildCalcButton("4"));
		button2.add(buildCalcButton("5"));
		button2.add(buildCalcButton("6"));
		button2.add(buildCalcButton("-"));

		button3.add(buildCalcButton("7"));
		button3.add(buildCalcButton("8"));
		button3.add(buildCalcButton("9"));
		button3.add(buildCalcButton("x"));


		button4.add(buildCalcButton("."));
		button4.add(buildCalcButton("0"));
		button4.add(buildCalcButton("÷"));
		VirtualButton equals = ButtonSelector.createButton("=");
		equals.blockActionEventForwarding(true);
		equals.addActionListener(new CalculatorListener(field));
		button4.add(equals);*/
